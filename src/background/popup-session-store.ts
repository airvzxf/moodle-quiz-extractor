...
function sanitize(state: PopupSessionState): PopupSessionState {
  // Redact user-provided text fields only.
  //
  // We deliberately do NOT redact `lastDocumentJson`:
  //  - It is our own structured output (QuizDocument built by
  //    `buildQuizDocument` and validated by `QuizDocumentSchema`).
  //  - The schema contains 64-char hex strings (the `stableFingerprint`
  //    of every question) that would always trip the `long-hex-blob`
  //    canary in the redactor. Those are NOT secrets — they are our
  //    own SHA-256 hashes of the question content.
  //  - Any leak that survived `buildQuizDocument` would also surface in
  //    the ZIP that we ship to disk (`tests/integration/zip-pipeline.spec.ts`
  //    catches that case), so the storage.session copy is no worse.
  //  - storage.session lives in the extension's sandbox anyway; the
  //    data does not cross a trust boundary.
  return {
    schemaVersion: POPUP_SESSION_SCHEMA_VERSION,
    originHash: state.originHash,
    savedAt: state.savedAt,
    answersText: redactString(state.answersText),
    lastDocumentJson: state.lastDocumentJson,
